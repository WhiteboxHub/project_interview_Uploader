...
async function transcribeVideo(videoPath, outputPath, onProgress = null) {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      // Convert video to WAV first with proper audio extraction
      const wavPath = path.join(outputDir, 'temp_audio.wav');
      console.log('🔄 Converting to WAV format...');
      
      const { exec } = require('child_process');
      // Extract audio properly: stereo, 16kHz, 16-bit PCM
      const ffmpegCmd = `ffmpeg -i "${videoPath}" -vn -acodec pcm_s16le -ar 16000 -ac 2 "${wavPath}" -y`;
      ...
      await new Promise((res, rej) => {
        exec(ffmpegCmd, (error, stdout, stderr) => {
          if (error) {
            ...
            console.log(`✅ Converted to WAV (${(stats.size / 1024 / 1024).toFixed(2)} MB)`);
            if (stats.size < 1000) {
